...
type
  ...
  TDL_tb_grupos_detalle = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'tb_grupos_detalle';
    ClaseNombre = 'TDL_tb_grupos_detalle';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{ TDL_tb_grupos_detalle }

{$REGION 'Metodos'}
constructor TDL_tb_grupos_detalle.Create;
begin
  Dataset := TDatasetMem.Create(nil);
  with Dataset do
  begin
    FileName := 'tb_grupos_detalle';
    FieldDefs.Clear;
...
destructor TDL_tb_grupos_detalle.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_tb_grupos_detalle.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_TB_GruposDetalle_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_tb_grupos_detalle.Insertar(var pResultado: Integer;
...
begin
  ...
  dmObjects.DB_TB_GruposDetalle_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_tb_grupos_detalle.Modificar(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_GruposDetalle_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_tb_grupos_detalle.Eliminar(pWhere: string;
  var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_GruposDetalle_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_tb_grupos_detalle.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_tb_grupos_detalle.Existe_Id(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_GruposDetalle_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_tb_grupos_detalle.Obtener_Id(pWhere: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_GruposDetalle_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_tb_grupos_detalle.Existe_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_GruposDetalle_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_tb_grupos_detalle.Obtener_Valor(pWhere, pCampo: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_GruposDetalle_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_tb_grupos_detalle.Actualizar_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_GruposDetalle_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
