unit uDL_TB_VENDEDORESCLIENTES;
...
type
  ...
  TDL_TB_VENDEDORESCLIENTES = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'TB_VENDEDORESCLIENTES';
    ClaseNombre = 'TDL_TB_VENDEDORESCLIENTES';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{ TDL_TB_VENDEDORESCLIENTES }

{$REGION 'Metodos'}
constructor TDL_TB_VENDEDORESCLIENTES.Create;
begin
  Dataset := TDatasetMem.Create(nil);
  with Dataset do
  begin
    FileName := 'TB_VENDEDORESCLIENTES';
    FieldDefs.Clear;
    FieldDefs.Add('ID', ftInteger, 0);
    FieldDefs.Add('IDVENDEDOR', ftInteger, 0);
    FieldDefs.Add('IDCLIENTE', ftInteger, 0);
    FieldDefs.Add('ClienteCODIGO', ftString, 100);
    FieldDefs.Add('ClienteNOMBRECOMPLETO', ftString, 160);
    CreateDataSet;
    Open;
  end;
...
destructor TDL_TB_VENDEDORESCLIENTES.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_TB_VENDEDORESCLIENTES.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_TB_VENDEDORESCLIENTES_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_TB_VENDEDORESCLIENTES.Insertar(var pResultado: Integer;
...
begin
  ...
  dmObjects.DB_TB_VENDEDORESCLIENTES_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_VENDEDORESCLIENTES.Modificar(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_VENDEDORESCLIENTES_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_VENDEDORESCLIENTES.Eliminar(pWhere: string;
  var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_VENDEDORESCLIENTES_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_VENDEDORESCLIENTES.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_TB_VENDEDORESCLIENTES.Existe_Id(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_VENDEDORESCLIENTES_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_VENDEDORESCLIENTES.Obtener_Id(pWhere: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_VENDEDORESCLIENTES_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_VENDEDORESCLIENTES.Existe_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_VENDEDORESCLIENTES_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_VENDEDORESCLIENTES.Obtener_Valor(pWhere, pCampo: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_VENDEDORESCLIENTES_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_VENDEDORESCLIENTES.Actualizar_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_VENDEDORESCLIENTES_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
