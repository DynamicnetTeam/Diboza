...
type
  ...
  TDL_TB_ENTRADASDETALLE = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'TB_ENTRADASDETALLE';
    ClaseNombre = 'TDL_TB_ENTRADASDETALLE';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{ TDL_TB_ENTRADASDETALLE}

{$REGION 'Metodos'}
constructor TDL_TB_ENTRADASDETALLE.Create;
begin
  Dataset := TDatasetMem.Create(nil);
  with Dataset do
  begin
    FileName := 'TB_ENTRADASDETALLE';
    FieldDefs.Clear;
    ...
    FieldDefs.Add('CANTIDAD', ftSingle, 0);
    FieldDefs.Add('CODIGOUNIDADMEDIDA', ftInteger, 0);
    FieldDefs.Add('NOMBREUNIDADMEDIDA', ftString, 100);
    FieldDefs.Add('PRECIOUNITARIO', ftSingle, 0);
    FieldDefs.Add('DESCUENTOP', ftSingle, 0);
    FieldDefs.Add('DESCUENTOM', ftSingle, 0);
    FieldDefs.Add('IVAP', ftSingle, 0);
    FieldDefs.Add('IVAM', ftSingle, 0);
    FieldDefs.Add('PRECIOUNITARIOFINAL', ftSingle, 0);
    FieldDefs.Add('PRECIOFINAL', ftSingle, 0);
...
destructor TDL_TB_ENTRADASDETALLE.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_TB_ENTRADASDETALLE.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_TB_ENTRADASDETALLE_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_TB_ENTRADASDETALLE.Insertar(var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
begin
  if FTypeConection = TTypeConection.Main then
    dmObjects.FDQuery1.Connection := dmConnection.FDMainConnection;
  if FTypeConection = TTypeConection.Local then
  ...
  dmObjects.DB_TB_ENTRADASDETALLE_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_ENTRADASDETALLE.Modificar(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_ENTRADASDETALLE_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_ENTRADASDETALLE.Eliminar(pWhere: string; var pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_ENTRADASDETALLE_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_ENTRADASDETALLE.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_TB_ENTRADASDETALLE.Existe_Id(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_ENTRADASDETALLE_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_ENTRADASDETALLE.Obtener_Id(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_ENTRADASDETALLE_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_ENTRADASDETALLE.Existe_Campo(pWhere, pCampo: string; pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection): Boolean;
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_ENTRADASDETALLE_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_ENTRADASDETALLE.Obtener_Valor(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_ENTRADASDETALLE_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_ENTRADASDETALLE.Actualizar_Campo(pWhere, pCampo: string; pValor: Variant;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_ENTRADASDETALLE_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
    pCampo, pValor, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_ENTRADASDETALLE.Sumar_Existencias(pWhere: string;
  d1: TFDMemTable; var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_ENTRADASDETALLE_Sumar_Existencias(dmObjects.FDQuery1,
    mWhere, Tabla, d1, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_ENTRADASDETALLE.Restar_Existencias(pWhere: string;
  d1: TFDMemTable; var pResultado: Integer; var pErrorM: string;
  FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_ENTRADASDETALLE_Restar_Existencias(dmObjects.FDQuery1,
    mWhere, Tabla, d1, ClaseNombre, pResultado, pErrorM);
end;
{$ENDREGION}
