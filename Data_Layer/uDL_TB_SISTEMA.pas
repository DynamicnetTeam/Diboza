...
type
  ...
  TDL_TB_SISTEMA = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'TB_SISTEMA';
    ClaseNombre = 'TDL_TB_SISTEMA';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{$REGION 'Metodos'}
constructor TDL_TB_SISTEMA.Create;
begin
  ...
  begin
    FileName := 'TB_SISTEMA';
    FieldDefs.Clear;
    FieldDefs.Add('VERSIONS', ftString, 100);
    FieldDefs.Add('IMAGENFONDO', ftBlob, 0);
    FieldDefs.Add('NOMBREIMAGEN', ftString, 255);
    FieldDefs.Add('IVADEFAULT', ftInteger, 0);
    FieldDefs.Add('ISERVICIO', ftFloat, 0);
    FieldDefs.Add('MONEDADEFAULT', ftInteger, 0);
    FieldDefs.Add('FAMILIADEFAULT', ftInteger, 0);
    FieldDefs.Add('UNIDADMEDIDADEFAULT', ftInteger, 0);
    FieldDefs.Add('MINIMODEFAULT', ftInteger, 0);
    FieldDefs.Add('APLICARMINIMODEFAULT', ftInteger, 0);
    FieldDefs.Add('APLICARREDONDEO', ftInteger, 0);
    FieldDefs.Add('APLICARREDONDEO5', ftInteger, 0);
    FieldDefs.Add('Clave', ftString, 100);
    FieldDefs.Add('MontoInicial', ftFloat, 0);
    FieldDefs.Add('PRODUCTOIMPUESTOINCLUIDO', ftInteger, 0);
    FieldDefs.Add('USARSEGURIDAD', ftInteger, 0);
    FieldDefs.Add('USARCONTABILIDAD', ftInteger, 0);
    FieldDefs.Add('TIPOMAQUINA', ftInteger, 0);
    FieldDefs.Add('IdVendedor', ftInteger, 0);
    FieldDefs.Add('AplicarBajarInventarioNC', ftInteger, 0);
    CreateDataSet;
    Open;
  end;
...
destructor TDL_TB_SISTEMA.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_TB_SISTEMA.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_TB_SISTEMA_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_TB_SISTEMA.Insertar(var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
begin
  if FTypeConection = TTypeConection.Main then
    dmObjects.FDQuery1.Connection := dmConnection.FDMainConnection;
  if FTypeConection = TTypeConection.Local then
  ...
  dmObjects.DB_TB_SISTEMA_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_SISTEMA.Modificar(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_SISTEMA_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_SISTEMA.Eliminar(pWhere: string; var pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_SISTEMA_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_SISTEMA.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_TB_SISTEMA.Existe_Id(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_SISTEMA_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_SISTEMA.Obtener_Id(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_SISTEMA_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_SISTEMA.Existe_Campo(pWhere, pCampo: string; pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection): Boolean;
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_SISTEMA_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_SISTEMA.Obtener_Valor(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_SISTEMA_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_SISTEMA.Actualizar_Campo(pWhere, pCampo: string; pValor: Variant;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_SISTEMA_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
