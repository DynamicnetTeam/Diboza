unit uDL_TB_DOCUMENTOSHACIENDA;
...
type
  ...
  TDL_TB_DOCUMENTOSHACIENDA = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'TB_DOCUMENTOSHACIENDA';
    ClaseNombre = 'TDL_DOCUMENTOSHACIENDA';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{ TDL_TB_DOCUMENTOSHACIENDA }

{$REGION 'Metodos'}
constructor TDL_TB_DOCUMENTOSHACIENDA.Create;
begin
  Dataset := TDatasetMem.Create(nil);
  with Dataset do
  begin
    FileName := 'TB_DOCUMENTOSHACIENDA';
    FieldDefs.Clear;
...
destructor TDL_TB_DOCUMENTOSHACIENDA.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_TB_DOCUMENTOSHACIENDA.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_TB_DOCUMENTOSHACIENDA_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_TB_DOCUMENTOSHACIENDA.Insertar(var pResultado: Integer;
...
begin
  ...
  dmObjects.DB_TB_DOCUMENTOSHACIENDA_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_DOCUMENTOSHACIENDA.Modificar(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_DOCUMENTOSHACIENDA_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_DOCUMENTOSHACIENDA.Eliminar(pWhere: string;
  var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_DOCUMENTOSHACIENDA_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_DOCUMENTOSHACIENDA.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_TB_DOCUMENTOSHACIENDA.Existe_Id(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_DOCUMENTOSHACIENDA_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_DOCUMENTOSHACIENDA.Obtener_Id(pWhere: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_DOCUMENTOSHACIENDA_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_DOCUMENTOSHACIENDA.Existe_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_DOCUMENTOSHACIENDA_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_DOCUMENTOSHACIENDA.Obtener_Valor(pWhere, pCampo: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_DOCUMENTOSHACIENDA_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_DOCUMENTOSHACIENDA.Actualizar_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_DOCUMENTOSHACIENDA_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
