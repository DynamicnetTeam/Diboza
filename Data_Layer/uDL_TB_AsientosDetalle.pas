...
type
  ...
  TDL_TB_AsientosDetalle = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'TB_AsientosDetalle';
    ClaseNombre = 'TDL_TB_AsientosDetalle';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{ TDL_TB_AsientosDetalle }

{$REGION 'Metodos'}
constructor TDL_TB_AsientosDetalle.Create;
begin
  Dataset := TDatasetMem.Create(nil);
  with Dataset do
  begin
    FileName := 'TB_AsientosDetalle';
    FieldDefs.Clear;
    FieldDefs.Add('ID', ftInteger, 0);
    FieldDefs.Add('IDENCABEZADO', ftInteger, 0);
    FieldDefs.Add('IDMOVIMIENTO', ftInteger, 0);
    FieldDefs.Add('IDCUENTA', ftInteger, 0);
    FieldDefs.Add('DESCRIPCION', ftBlob, 0);
    FieldDefs.Add('REFERENCIA', ftString, 100);
    FieldDefs.Add('MONTO', ftFloat, 0);
    FieldDefs.Add('DEBE', ftFloat, 0);
    FieldDefs.Add('HABER', ftFloat, 0);
    FieldDefs.Add('LINEA', ftInteger, 0);
    FieldDefs.Add('NOTAS', ftMemo, 0);
    FieldDefs.Add('IDTIPOORIGEN', ftInteger, 0);
    FieldDefs.Add('IDORIGEN', ftInteger, 0);
    FieldDefs.Add('CuentaNombre', ftString, 255);
    FieldDefs.Add('MovimientoNombre', ftString, 255);
    CreateDataSet;
    Open;
  end;
...
destructor TDL_TB_AsientosDetalle.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_TB_AsientosDetalle.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_TB_AsientosDetalle_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_TB_AsientosDetalle.Insertar(var pResultado: Integer;
...
begin
  ...
  dmObjects.DB_TB_AsientosDetalle_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_AsientosDetalle.Modificar(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_AsientosDetalle_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_AsientosDetalle.Eliminar(pWhere: string;
  var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_AsientosDetalle_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_AsientosDetalle.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_TB_AsientosDetalle.Existe_Id(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_AsientosDetalle_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_AsientosDetalle.Obtener_Id(pWhere: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_AsientosDetalle_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_AsientosDetalle.Existe_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_AsientosDetalle_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_AsientosDetalle.Obtener_Valor(pWhere, pCampo: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_AsientosDetalle_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_AsientosDetalle.Actualizar_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_AsientosDetalle_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
