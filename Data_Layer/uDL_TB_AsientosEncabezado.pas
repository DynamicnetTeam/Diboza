unit uDL_TB_AsientosEncabezado;
...
type
  ...
  TDL_TB_AsientosEncabezado = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'TB_AsientosEncabezado';
    ClaseNombre = 'TDL_TB_AsientosEncabezado';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{ TDL_TB_AsientosEncabezado }

{$REGION 'Metodos'}
constructor TDL_TB_AsientosEncabezado.Create;
begin
  Dataset := TDatasetMem.Create(nil);
  with Dataset do
  begin
    FileName := 'TB_AsientosEncabezado';
    FieldDefs.Clear;
    FieldDefs.Add('ID', ftInteger, 0);
    FieldDefs.Add('FECHACREACION', ftDate, 0);
    FieldDefs.Add('FECHA', ftDate, 0);
    FieldDefs.Add('NUMERO', ftString, 200);
    FieldDefs.Add('CODIGOESTADO', ftInteger, 0);
    FieldDefs.Add('CODIGOCIERRE', ftInteger, 0);
    ...
    FieldDefs.Add('EstadoNombre', ftString, 200);
    CreateDataSet;
    Open;
  end;
...
destructor TDL_TB_AsientosEncabezado.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_TB_AsientosEncabezado.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_TB_AsientosEncabezado_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_TB_AsientosEncabezado.Insertar(var pResultado: Integer;
...
begin
  ...
  dmObjects.DB_TB_AsientosEncabezado_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_AsientosEncabezado.Modificar(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_AsientosEncabezado_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_AsientosEncabezado.Eliminar(pWhere: string;
  var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_AsientosEncabezado_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_AsientosEncabezado.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_TB_AsientosEncabezado.Existe_Id(pWhere: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_AsientosEncabezado_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_AsientosEncabezado.Obtener_Id(pWhere: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_AsientosEncabezado_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_AsientosEncabezado.Existe_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_AsientosEncabezado_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_AsientosEncabezado.Obtener_Valor(pWhere, pCampo: string;
  var pResultado: Integer; var pErrorM: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_AsientosEncabezado_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_AsientosEncabezado.Actualizar_Campo(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_AsientosEncabezado_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
