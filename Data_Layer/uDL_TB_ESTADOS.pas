...
type
  ...
  TDL_TB_ESTADOS = class
    Dataset: TDatasetMem;
    _ListaCampos: TStringList;
  const
    Tabla = 'tb_estados';
    ClaseNombre = 'TDL_TB_ESTADOS';
  public
    {$REGION 'Metodos'}
    constructor Create;
    destructor Destroy;
...
{$REGION 'Metodos'}
constructor TDL_TB_ESTADOS.Create;
begin
...
destructor TDL_TB_ESTADOS.Destroy;
begin
  Dataset.Close;
  Dataset.Free;
  _ListaCampos.Free;
...
procedure TDL_TB_ESTADOS.Consultar(var pResultado: Integer;
...
begin
  ...
  if Trim(pSelect) = '' then
    dmObjects.DB_TB_ESTADOS_Consultar(dmObjects.FDQuery1, Dataset,
...
procedure TDL_TB_ESTADOS.Insertar(var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
begin
  if FTypeConection = TTypeConection.Main then
    dmObjects.FDQuery1.Connection := dmConnection.FDMainConnection;
  if FTypeConection = TTypeConection.Local then
  ...
  dmObjects.DB_TB_ESTADOS_Insertar(dmObjects.FDQuery1, Dataset, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_ESTADOS.Modificar(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_ESTADOS_Modificar(dmObjects.FDQuery1, Dataset, mWhere,
    ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_ESTADOS.Eliminar(pWhere: string; var pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_ESTADOS_Eliminar(dmObjects.FDQuery1, mWhere, Tabla, ClaseNombre,
    pResultado, pErrorM);
end;

procedure TDL_TB_ESTADOS.Limpiar_Tag;
begin
  dmObjects.DB_Limpiar_Tag(Dataset);
end;

function TDL_TB_ESTADOS.Existe_Id(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_ESTADOS_Existe_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_ESTADOS.Obtener_Id(pWhere: string; var pResultado: Integer;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_ESTADOS_Obtener_Id(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_ESTADOS.Existe_Campo(pWhere, pCampo: string; pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection): Boolean;
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_ESTADOS_Existe_Campo(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_ESTADOS.Obtener_Valor(pWhere, pCampo: string;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_ESTADOS_Obtener_Valor(dmObjects.FDQuery1, mWhere, Tabla,
      pCampo, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_ESTADOS.Actualizar_Campo(pWhere, pCampo: string; pValor: Variant;
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_ESTADOS_Actualizar_Campo(dmObjects.FDQuery1, mWhere, Tabla,
