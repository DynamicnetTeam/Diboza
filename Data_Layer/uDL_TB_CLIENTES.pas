...
procedure TDL_TB_CLIENTES.Insertar(var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
begin
  if FTypeConection = TTypeConection.Main then
    dmObjects.FDQuery1.Connection := dmConnection.FDMainConnection;
  if FTypeConection = TTypeConection.Local then
...
procedure TDL_TB_CLIENTES.Sumar_Saldo(pWhere: string; pValor: Double;
  var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_CLIENTES_Sumar_Saldo(dmObjects.FDQuery1, mWhere, Tabla,
    pValor, ClaseNombre, pResultado, pErrorM);
end;

procedure TDL_TB_CLIENTES.Restar_Saldo(pWhere: string; pValor: Double;
  var pResultado: Integer; var pErrorM: string; FTypeConection: TTypeConection);
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  dmObjects.DB_TB_CLIENTES_Restar_Saldo(dmObjects.FDQuery1, mWhere, Tabla,
    pValor, ClaseNombre, pResultado, pErrorM);
end;
{$ENDREGION}
