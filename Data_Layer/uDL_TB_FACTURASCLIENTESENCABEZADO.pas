...
type
  ...
  public
    ...
    procedure Estadistico(pWhere: string; pds1: TDatasetMem;
      var pResultado: Integer;
      var pErrorM: string; FTypeConection: TTypeConection = udmConnection.Main);
    function Obtener_TotalDescuentos(pWhere: string;
      var pResultado: Integer;
      var pErrorM: string; FTypeConection: TTypeConection = udmConnection.Main): Double;
    function Obtener_TotalKilos(pWhere: string;
      var pResultado: Integer;
      var pErrorM: string; FTypeConection: TTypeConection = udmConnection.Main): Double;
    {$ENDREGION}
  end;
...
begin
  ...
  begin
    ...
    FieldDefs.Add('NUMERONotaCredito', ftInteger, 0);
...
function TDL_TB_FACTURASCLIENTESENCABEZADO.Obtener_TotalDescuentos(
...
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_FACTURASCLIENTESENCABEZADO_Obtener_TotalDescuentos(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;

function TDL_TB_FACTURASCLIENTESENCABEZADO.Obtener_TotalKilos(pWhere: string;
  var pResultado: Integer;
  var pErrorM: string; FTypeConection: TTypeConection = udmConnection.Main): Double;
var
  mWhere: string;
begin
  ...
  if Trim(pWhere) <> '' then
    mWhere := 'Where '+ pWhere;
  Result :=
    dmObjects.DB_TB_FACTURASCLIENTESENCABEZADO_Obtener_TotalKilos(dmObjects.FDQuery1, mWhere, Tabla,
      ClaseNombre, pResultado, pErrorM);
end;
{$ENDREGION}
