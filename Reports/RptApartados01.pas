...
var
  ReptApartados01: TReptApartados01;
...
uses
  uSistema, ufunciones, uDL_TB_APARTADOSENCABEZADO, BS_DBConexion, DL_DBGeneral,
  uDL_TB_CLIENTES, udmLenguajes, FrmVerReporte01;

{$R *.dfm}

var
  Forma01: TReptApartados01;

{$REGION 'Funciones Forma'}
procedure TReptApartados01.FormShow(Sender: TObject);
begin
  Forma01 := ReptApartados01;

  InicializarForma;
end;

procedure TReptApartados01.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FinalizarForma;
end;

procedure TReptApartados01.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  KeyDownForma(Key, Shift);
end;

procedure TReptApartados01.btnAceptarClick(Sender: TObject);
begin
  Aceptar;
end;

procedure TReptApartados01.btnCancelarClick(Sender: TObject);
begin
  Cancelar;
end;
{$ENDREGION}
...
begin
  ...
  begin
    ...
    try
      ...
      mDL_TB_APARTADOSENCABEZADO := TDL_TB_APARTADOSENCABEZADO.Create;
      mDL_TB_APARTADOSENCABEZADO.Consultar(_Resultado, _ErrorM,
        mWhere.Text, mOrderBy.Text);
      mWhere.Free;
      mOrderBy.Free;
      if _Resultado = -1 then
        raise Exception.Create('');
      with mDL_TB_APARTADOSENCABEZADO.Dataset do
      begin
        First;
        while not Eof do
        begin
      ...
      mDL_TB_APARTADOSENCABEZADO.Destroy;
