...
type
  TFormRecibosProveedores01 = class(TForm)
...
var
  FormRecibosProveedores01: TFormRecibosProveedores01;
...
uses
  uSistema, ufunciones, uDL_TB_RECIBOSPROVEEDORESENCABEZADO, BS_DBConexion,
  FrmMensaje_Espera01, uDL_TB_PRODUCTOS, uDL_TB_RECIBOSPROVEEDORESDETALLE,
  uDL_TB_PROVEEDORES, FrmMantenimientoProveedores01,
  uDL_TB_CONSECUTIVOS, uDL_TB_PLANTILLAS, uDL_TB_ENTRADASENCABEZADO,
...
type
  Clase_Tipo1 = TDL_TB_RECIBOSPROVEEDORESENCABEZADO;
  Clase_Tipo2 = TDL_TB_RECIBOSPROVEEDORESDETALLE;
...
var
  Forma01: TFormRecibosProveedores01;
  _NumeroFactura: Integer;
  _DL_TB_FACTURASDETALLEActual: Clase_Tipo2;
  _Total: Double;

{$REGION 'Funciones Forma'}
procedure TFormRecibosProveedores01.FormShow(Sender: TObject);
begin
  Forma01 := FormRecibosProveedores01;

  InicializarForma;
end;

procedure TFormRecibosProveedores01.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  FinalizarForma;
end;

procedure TFormRecibosProveedores01.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  KeyDownForma(Key, Shift);
end;

procedure TFormRecibosProveedores01.btnNuevoClick(Sender: TObject);
begin
  Nuevo;
end;

procedure TFormRecibosProveedores01.btnPagarClick(Sender: TObject);
begin
  Pagar_Data;
end;

procedure TFormRecibosProveedores01.btnAnularClick(Sender: TObject);
begin
  Anular_Data;
end;

procedure TFormRecibosProveedores01.btnCerrarClick(Sender: TObject);
begin
  Cerrar;
end;

procedure TFormRecibosProveedores01.btnBuscar_ClientesClick(Sender: TObject);
begin
  FormMantenimientoProveedores01.ShowModal;
  LlenarcbClientes(cbCliente);
end;

procedure TFormRecibosProveedores01.btnInsertarLineaClick(Sender: TObject);
begin
  Insertar_Detalle;
end;

procedure TFormRecibosProveedores01.btnEliminarLineaClick(Sender: TObject);
begin
  Eliminar_Detalle;
end;

procedure TFormRecibosProveedores01.cbClienteChange(Sender: TObject);
begin
  if _IdEstado = 1 then
    Consultar_Cliente(GetCombo(cbCliente));
end;

procedure TFormRecibosProveedores01.chkTodoLista1Click(Sender: TObject);
begin
  MDLista_Todos(lvLista1, chkTodoLista1);
end;

procedure TFormRecibosProveedores01.cbDocumentosDropDown(Sender: TObject);
begin
  cbDocumentos.Perform(CB_SETDROPPEDWIDTH, 600, 0);
end;

procedure TFormRecibosProveedores01.cbDocumentosChange(Sender: TObject);
begin
  Consultar_Documento(GetCombo(cbDocumentos));
end;

procedure TFormRecibosProveedores01.cbClienteDropDown(Sender: TObject);
begin
...
var
  ...
  mDL_TB_FACTURASCLIENTESENCABEZADO: TDL_TB_ENTRADASENCABEZADO;
begin
  with Forma01 do
  begin
    ...
    try
      ...
      mWhere := TStringList.Create;
      mOrderBy := TStringList.Create;
      mWhere.Add('A.IdProveedor=' + IntToStr(pId));
      mWhere.Add(' And ');
      mWhere.Add('A.CODIGOFORMAPAGO=5');
      mWhere.Add(' And ');
      mWhere.Add('A.CODIGOESTADO=4');
      mOrderBy.Add('A.FECHAVENCIMIENTO');
      mDL_TB_FACTURASCLIENTESENCABEZADO := TDL_TB_ENTRADASENCABEZADO.Create;
...
begin
  ...
  begin
    ...
    try
      ...
      mNumero :=
        mDL_TB_CONSECUTIVOS.Obtener_Valor('', 'RECIBOSABONOSPROVEEDORES',
      _Resultado, _ErrorM);
      if _Resultado = -1 then
        raise Exception.Create('');
      ...
      mDL_TB_CONSECUTIVOS := TDL_TB_CONSECUTIVOS.Create;
      mDL_TB_CONSECUTIVOS.Actualizar_Campo('', 'RECIBOSABONOSPROVEEDORES', mNumero,
        _Resultado, _ErrorM);
      if _Resultado = -1 then
        raise Exception.Create('');
      mDL_TB_CONSECUTIVOS.Destroy;
      ...
      mDL_TB_PROVEEDORES := TDL_TB_PROVEEDORES.Create;
      mDL_TB_PROVEEDORES.Restar_Saldo(mWhere.Text, _Total,
        _Resultado, _ErrorM);
      if _Resultado = -1 then
        raise Exception.Create('');
      mWhere.Free;
...
begin
  ...
  begin
    ...
    try
      ...
      mWhere.Add('Id=' + IntToStr(GetCombo(cbCliente)));
      mDL_TB_PROVEEDORES := TDL_TB_PROVEEDORES.Create;
      mDL_TB_PROVEEDORES.Sumar_Saldo(mWhere.Text, _Total,
        _Resultado, _ErrorM);
      if _Resultado = -1 then
        raise Exception.Create('');
      mWhere.Free;
...
begin
  ...
  begin
    ...
    try
      ...
      mDL_TB_PROVEEDORES := TDL_TB_PROVEEDORES.Create;
      mSaldo :=
        mDL_TB_PROVEEDORES.Obtener_Valor('Id=' + IntToStr(mIdCliente), 'Saldo', _Resultado, _ErrorM);
      mDL_TB_PROVEEDORES.Destroy;
