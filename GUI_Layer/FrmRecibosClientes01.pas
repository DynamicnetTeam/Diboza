...
type
  TFormRecibosClientes01 = class(TForm)
...
var
  FormRecibosClientes01: TFormRecibosClientes01;
...
uses
  uSistema, ufunciones, uDL_TB_RECIBOSCLIENTESENCABEZADO, BS_DBConexion,
  FrmMensaje_Espera01, uDL_TB_PRODUCTOS, uDL_TB_RECIBOSCLIENTESDETALLE,
...
type
  Clase_Tipo1 = TDL_TB_RECIBOSCLIENTESENCABEZADO;
  Clase_Tipo2 = TDL_TB_RECIBOSCLIENTESDETALLE;
...
var
  Forma01: TFormRecibosClientes01;
  _NumeroFactura: Integer;
  _DL_TB_FACTURASDETALLEActual: Clase_Tipo2;
  _Total: Double;

{$REGION 'Funciones Forma'}
procedure TFormRecibosClientes01.FormShow(Sender: TObject);
begin
  Forma01 := FormRecibosClientes01;

  InicializarForma;
end;

procedure TFormRecibosClientes01.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  FinalizarForma;
end;

procedure TFormRecibosClientes01.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  KeyDownForma(Key, Shift);
end;

procedure TFormRecibosClientes01.btnNuevoClick(Sender: TObject);
begin
  Nuevo;
end;

procedure TFormRecibosClientes01.btnPagarClick(Sender: TObject);
begin
  Pagar_Data;
end;

procedure TFormRecibosClientes01.btnAnularClick(Sender: TObject);
begin
  Anular_Data;
end;

procedure TFormRecibosClientes01.Imprimir1Click(Sender: TObject);
begin
  Imprimir_Data;
end;

procedure TFormRecibosClientes01.VistaPrevia1Click(Sender: TObject);
begin
  VistaPrevia;
end;

procedure TFormRecibosClientes01.btnCerrarClick(Sender: TObject);
begin
  Cerrar;
end;

procedure TFormRecibosClientes01.btnBuscar_ClientesClick(Sender: TObject);
begin
  FormMantenimientoClientes01.ShowModal;
  LlenarcbClientes(cbCliente);
end;

procedure TFormRecibosClientes01.btnInsertarLineaClick(Sender: TObject);
begin
  Insertar_Detalle;
end;

procedure TFormRecibosClientes01.btnEliminarLineaClick(Sender: TObject);
begin
  Eliminar_Detalle;
end;

procedure TFormRecibosClientes01.cbClienteChange(Sender: TObject);
begin
  if _IdEstado = 1 then
    Consultar_Cliente(GetCombo(cbCliente));
end;

procedure TFormRecibosClientes01.chkTodoLista1Click(Sender: TObject);
begin
  MDLista_Todos(lvLista1, chkTodoLista1);
end;

procedure TFormRecibosClientes01.cbDocumentosDropDown(Sender: TObject);
begin
  cbDocumentos.Perform(CB_SETDROPPEDWIDTH, 600, 0);
end;

procedure TFormRecibosClientes01.cbDocumentosChange(Sender: TObject);
begin
  Consultar_Documento(GetCombo(cbDocumentos));
end;

procedure TFormRecibosClientes01.cbClienteDropDown(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    try
      ...
      mWhere := TStringList.Create;
      mOrderBy := TStringList.Create;
      mWhere.Add('A.IdCliente=' + IntToStr(pId));
      mWhere.Add(' And ');
...
begin
  ...
  begin
    ...
    try
      ...
      mNumero :=
        mDL_TB_CONSECUTIVOS.Obtener_Valor('', 'RECIBOSABONOSCLIENTES',
      _Resultado, _ErrorM);
      if _Resultado = -1 then
        raise Exception.Create('');
      ...
      mDL_TB_CONSECUTIVOS := TDL_TB_CONSECUTIVOS.Create;
      mDL_TB_CONSECUTIVOS.Actualizar_Campo('', 'RECIBOSABONOSCLIENTES', mNumero,
        _Resultado, _ErrorM);
      if _Resultado = -1 then
        raise Exception.Create('');
      mDL_TB_CONSECUTIVOS.Destroy;
...
procedure Anular_Data;
var
  mDL_TB_RecibosCLIENTESENCABEZADO: Clase_Tipo1;
  mDL_TB_FACTURASCLIENTESENCABEZADO: TDL_TB_FACTURASCLIENTESENCABEZADO;
  mDL_TB_CLIENTES: TDL_TB_CLIENTES;
  mWhere: TStringList;
  mResultado: Integer;
...
begin
  ...
  begin
    ...
    try
      ...
      mDL_TB_RecibosCLIENTESENCABEZADO := Clase_Tipo1.Create;
      mDL_TB_RecibosCLIENTESENCABEZADO.Actualizar_Estado(mWhere.Text, 3,
        _Resultado, _ErrorM);
      mWhere.Free;
      if _Resultado = -1 then
        raise Exception.Create('');
