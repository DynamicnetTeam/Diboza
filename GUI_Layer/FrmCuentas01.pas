...
begin
  ...
  begin
    ...
    mEncontrado :=
      mBS_Cuentas.Existe_Campo(mWhere.Text, 'Codigo', _Resultado, _ErrorM);
    mWhere.Free;
    mBS_Cuentas.Destroy;
    if _Opcion = 1 then
    begin
...
var
  ...
  mResultado: Integer;
  mErrorM: string;
  mPadreCodigoAnterior, mPadreCodigo: string;
begin
  with Forma01 do
  begin
    ...
    try
      ...
      mPadreCodigo :=
        mBS_Cuentas.Obtener_Valor('Id=' + IntToStr(mIdPadre),
        'CODIGO', _Resultado, _ErrorM);
      if _Resultado = -1 then
        raise Exception.Create('');
      mBS_Cuentas.Destroy;
      Inc(mNivel);
      if Trim(mPadreCodigoAnterior) <> '' then
        mPadreCodigo := mPadreCodigoAnterior + ',' + mPadreCodigo;
...
var
  ...
  mResultado: Integer;
  mErrorM: string;
  mIdPadre, mIdTipo, mNivel: Integer;
  mPadreCodigoAnterior, mPadreCodigo: string;
begin
  with Forma01 do
  begin
