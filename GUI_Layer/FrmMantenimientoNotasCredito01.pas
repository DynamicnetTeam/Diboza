unit FrmMantenimientoNotasCredito01;
...
type
  TFormMantenimientoNotasCredito01 = class(TForm)
    pnlTitulo: TRzPanel;
...
var
  FormMantenimientoNotasCredito01: TFormMantenimientoNotasCredito01;
...
procedure TFormMantenimientoNotasCredito01.FormShow(Sender: TObject);
begin
...
procedure TFormMantenimientoNotasCredito01.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  FinalizarForma;
end;

procedure TFormMantenimientoNotasCredito01.FormKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  KeyDownForma(Key, Shift);
end;

procedure TFormMantenimientoNotasCredito01.btnInsertarClick(Sender: TObject);
begin
  Insertar;
end;

procedure TFormMantenimientoNotasCredito01.btnModificarClick(Sender: TObject);
begin
  Modificar(lvLista1);
end;

procedure TFormMantenimientoNotasCredito01.btnRefrescarClick(Sender: TObject);
begin
  Refrescar(lvLista1);
end;

procedure TFormMantenimientoNotasCredito01.btnCerrarClick(Sender: TObject);
begin
  Cerrar;
end;

procedure TFormMantenimientoNotasCredito01.edtBuscar_NumeroInvokeSearch(
  Sender: TObject);
begin
  Buscar_Numero(edtBuscar_Numero.Text, lvLista1);
end;

procedure TFormMantenimientoNotasCredito01.lvLista1CustomDrawItem(
...
begin
  ...
  begin
    ...
    FormNotasCredito01._Origen := 1;
    FormNotasCredito01._IdEstado := 1;
    FormNotasCredito01._IdFormaPago := 0;
    FormNotasCredito01._IdDocumento := 0;
    FormNotasCredito01._Numero := 0;
    FormNotasCredito01.WindowState := wsMaximized;
...
begin
  ...
  begin
    ...
    begin
      ...
      mLinea := lv.Selected.SubItems[5];
      try
        mEstado := StrToInt(mLinea);
      except
        mEstado := 0;
      end;
      if (mId > 0) and (mEstado > 0) then
      begin
        mDocumento := mId;
        FormNotasCredito01._Origen := 1;
        FormNotasCredito01._IdEstado := mEstado;
        FormNotasCredito01._IdFormaPago := mFormaPago;
        FormNotasCredito01._IdDocumento := mId;
        FormNotasCredito01._Numero := mDocumento;
        FormNotasCredito01.WindowState := wsMaximized;
        FormNotasCredito01.ShowModal;
        btnRefrescar.Click;
      end;
...
begin
  ...
  begin
    ...
    try
      ...
      mDL_TB_NOTASCREDITOENCABEZADO := Clase_Tipo1.Create;
      ...
      LlenarlvLista1(lv, mDL_TB_NOTASCREDITOENCABEZADO.Dataset);
